...
def training(args, dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    if args.use_dust3r:
        pose_json = args.dust3r_json or os.path.join(
            dataset.source_path, f'dust3r_{args.sparse_view_num}.json'
        )
        with open(pose_json, 'r') as f:
            json_cameras = json.load(f)
        refined_cameras = []
        json_cameras_by_id = {camera['id']: camera for camera in json_cameras}
        for viewpoint_cam in scene.getTrainCameras():
            camera = dict(json_cameras_by_id[viewpoint_cam.uid])
            w2c = np.eye(4)
        ...
        refined_cameras.sort(key=lambda camera: camera['id'])
