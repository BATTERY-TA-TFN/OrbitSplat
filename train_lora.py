...
def resolve_model_path(filename: str) -> str:
    ...
    if os.path.exists(local_path):
        return local_path
    external_dir = os.environ.get('GAUSSIANOBJECT_MODEL_DIR')
    if external_dir:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--disable_image_logger', action='store_true', default=False)

    args = parser.parse_args()

    model = create_model(f'./models/{args.model_name}.yaml').cpu()
    model.load_state_dict(load_state_dict(resolve_model_path('v1-5-pruned.ckpt'), location='cpu'), strict=False)
    model.load_state_dict(load_state_dict(resolve_model_path(f'{args.model_name}.pth'), location='cpu'), strict=False)
    ...
    if args.train_lora:
        ...
        for parameter in get_lora_params(model):
    ...
    callbacks = [LoraCheckpoint(exp_dir=exp_path, every_n_train_steps=args.callbacks_every_n_train_steps)]
    if not args.disable_image_logger:
        callbacks.insert(
            0,
            ImageLogger(
                exp_dir=exp_path,
                every_n_train_steps=args.callbacks_every_n_train_steps,
