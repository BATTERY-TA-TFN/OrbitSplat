...
def render_path(dataset : ModelParams, iteration : int, pipeline : PipelineParams, extra_opts=None):
    ...
    depths = []
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        if args.render_resize_method == 'original':
            image_width = view.image_width
            image_height = view.image_height
        elif args.render_resize_method == 'crop':
            image_size = extra_opts.render_size
        ...
        focal_length_x = fov2focal(view.FoVx, view.image_width) * extra_opts.render_focal_scale
        focal_length_y = fov2focal(view.FoVy, view.image_height) * extra_opts.render_focal_scale
    ...
    if not extra_opts.not_generate_video:
        renders_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders.mp4")
        subprocess.run(["ffmpeg", "-y",
                    "-framerate", "24",
                    "-i", os.path.join(render_path, "%05d.png"),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--render_path_start", type=int, default=0)
    parser.add_argument("--render_path_end", type=int, default=0)
    parser.add_argument(
        "--render_resize_method",
        default="crop",
        choices=("crop", "pad", "original"),
        help="Render square crop/pad views or preserve the source camera resolution and aspect ratio.",
    )
    ...
    parser.add_argument(
        ...
        default=1.0,
        type=float,
        help="Scale render-path focal length. Values below 1 zoom out and preserve framing margins.",
    )
    ### some exp args
