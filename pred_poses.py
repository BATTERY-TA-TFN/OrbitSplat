...
def resolve_model_path(filename):
    ...
    if external_model_dir:
        external_path = os.path.join(external_model_dir, filename)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--niter', type=int, default=300,
                        help='Number of global alignment iterations.')
    parser.add_argument('--masked_input', action='store_true',
                        help='Use masks to remove the static background during pose matching.')
    args = parser.parse_args()

    model_path = resolve_model_path('DUSt3R_ViTLarge_BaseDecoder_512_dpt.pth')
    device = 'cuda'
    batch_size = 1
    schedule = 'cosine'
    lr = 0.01
    ...
    original_masks = []
    for mask, image in zip(masks, original_images):
        mask_array = np.array(Image.open(mask).resize(image.size))
        if mask_array.ndim == 3:
            mask_array = mask_array[:, :, 0]
        original_masks.append(mask_array / 255.0)

    pose_images = images
    if args.masked_input:
        masked_dir = os.path.join(scene_path, f'pose_images_masked_{sparse_num}')
        os.makedirs(masked_dir, exist_ok=True)
        pose_images = []
        for image_path, image, mask in zip(images, original_images, original_masks):
            rgb = np.array(image.convert('RGB'))
            masked = np.where((mask > 0.5)[..., None], rgb, 255).astype(np.uint8)
            masked_path = os.path.join(masked_dir, os.path.basename(image_path))
            Image.fromarray(masked).save(masked_path)
            pose_images.append(masked_path)

    loaded_images = load_images(pose_images, size=512)
