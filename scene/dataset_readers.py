...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder, extra_opts=None):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        ### load masks
        mask_dir = getattr(extra_opts, "mask_dir", "masks")
        mask_path_png = osp.join(osp.dirname(images_folder), mask_dir, osp.basename(
...
def readColmapSceneInfo(path, images, eval, llffhold=8, extra_opts=None):
    ...
    render_source_infos = cam_infos
    render_path_start = getattr(extra_opts, "render_path_start", 0)
    render_path_end = getattr(extra_opts, "render_path_end", 0)
    if render_path_end > render_path_start:
        render_source_infos = cam_infos[render_path_start:render_path_end]
    render_cam_infos = generate_ellipse_path_from_camera_infos(render_source_infos)
    ...
    if hasattr(extra_opts, 'sparse_view_num') and extra_opts.sparse_view_num > 0: # means sparse setting
        ...
        sparse_test_path = osp.join(path, f"sparse_test_{str(extra_opts.sparse_view_num)}.txt")
        if not osp.exists(sparse_test_path):
            sparse_test_path = osp.join(path, "sparse_test.txt")
        ids_test = np.loadtxt(sparse_test_path, dtype=np.int32)
...
def readDust3rSceneInfo(path, images, eval, extra_opts=None):
    ...
    if hasattr(extra_opts, 'dust3r_json') and extra_opts.dust3r_json:
        ...
        with open(osp.join(path, f"dust3r_{str(extra_opts.sparse_view_num)}.json")) as f:
            dust3r_frames = json.load(f)
    depth_path = osp.join(path, f"dust3r_depth_{str(extra_opts.sparse_view_num)}.npy")
    confidence_path = osp.join(path, f"dust3r_confidence_{str(extra_opts.sparse_view_num)}.npy")
    train_cam_infos = readDust3rCameras(
        dust3r_frames, images_folder,
        depth_path if osp.exists(depth_path) else None,
        confidence_path if osp.exists(confidence_path) else None
    )
    if osp.exists(osp.join(path, f"dust3r_{str(extra_opts.sparse_view_num)}_test.json")):
        with open(osp.join(path, f"dust3r_{str(extra_opts.sparse_view_num)}_test.json")) as f:
            test_dust3r_frames = json.load(f)
