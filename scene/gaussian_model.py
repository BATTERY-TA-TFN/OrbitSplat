...
def knn_dists_with_optional_pytorch3d(xyz: torch.Tensor, k: int) -> torch.Tensor:
    if p3dops is not None:
        return p3dops.knn_points(
            xyz.unsqueeze(0),
            xyz.unsqueeze(0),
            K=k,
        ).dists

    sample_size = min(xyz.shape[0], max(2048, min(8192, k * 4)))
    sample_indices = torch.randperm(xyz.shape[0], device=xyz.device)[:sample_size]
    sample_xyz = xyz[sample_indices]
    chunk_size = 4096
    chunks = []
    topk = min(k, sample_size)
    for start in range(0, xyz.shape[0], chunk_size):
        dists = torch.cdist(xyz[start:start + chunk_size], sample_xyz).pow(2)
        chunks.append(torch.topk(dists, k=topk, largest=False, dim=-1).values)
...
class GaussianModel:
    ...
    def remove_outliers(self, opt, step, linear=False, removing_ratio=0, remaining_rate=0.0):
        ...
        nearest_k_distance = knn_dists_with_optional_pytorch3d(xyz, int(num_points**0.5))
